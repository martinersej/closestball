...
public class ClosestBall {
    public int computeClosest(ArrayList<Integer> players, ArrayList<Integer> balls) {
        ...
        int num = 0;
        int num_1 = 0;
        while (b < balls.size() && p < players.size()) {
            num = Integer.MAX_VALUE;
            num_1 = Integer.MAX_VALUE;
            try { 
                if (players.get(p) < balls.get(b) && players.get(p+1) > balls.get(b)) {
                    num = Math.abs(players.get(p) - balls.get(b));
                    num_1 = Math.abs(players.get(p+1) - balls.get(b));
                    b++;
                } else {
                    num = Math.abs(players.get(p) - balls.get(b));
                    if (players.get(p) > balls.get(b)) {
                        b++;
                    } else {
                        p++;
                    }
